  ...
  struct proc *curproc = myproc();
    ...
    if (curproc->priority != curproc->basepriority) {
      cprintf("[%d] priority restored to %d from inherited %d\n", 
              curproc->pid, curproc->basepriority, curproc->priority);
...
int
clone(void* fcn, void *arg, void *stack)
{
  struct proc *np;
  struct proc *curproc = myproc();
  ...
  *np->tf = *curproc->tf;
  ...
  // (stack is actually the botton of the page)
...
// Check the list of currently running processes, looking
// for a thread belonging to the parent process.
// if it finds such a process, it will "kill" it (clear its entry 
// in the process table) by setting it to the UNUSED state and resetting all
// of its values. it will then return the pid of the child thread that 
// was killed
int
join(void)
{
...
int
holding_t(struct ticketlock *lk)
{
  struct proc *curproc = myproc(); 
