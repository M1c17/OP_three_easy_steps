...
class ssd:
    ...
    def get_cursor(self):
        if self.current_page == -1:
            for block in list(range(self.current_block, self.num_blocks)) + list(range(0, self.current_block)):
    ...
    def garbage_collect(self):
        blocks_cleaned = 0
        for block in list(range(self.gc_current_block, self.num_blocks)) + list(range(0, self.gc_current_block)):
