...
key_valueNode *ht_pair(const char *key, const char *value)
{
    ...
    key_value->values = malloc(SIZE_ARRAY*sizeof(char*));
...
void *SortPartitionAndReduce()
{
    ...
    for(int j = 0; j < HASH_SIZE; ++j) {
        ...
        while(temp_key_valueNode) {
                ...
                //printf("this is current key: %s\n", temp_key_valueNode->key);
    ...
    //     printf("this is keys list: %s\n", list[x]->key);
    // }
    // printf("numkeys %lu\n", partitionHashT[current_partition]->numKeys);
...
char *get_next(char *key, int partition_number)
{   
    //printf("this is reducelist %p\n", reduceList[partition_number]);
    ...
    while(curr_keyValue != NULL) {
        if(strcmp(curr_keyValue->key, key) == 0) {;
            printf("HERRRRREEEE num Array: %d\n", curr_keyValue->num_valueArray);
            int current_value = curr_keyValue->num_valueArray;
            current_value--;
            return curr_keyValue->values[current_value+1];
        }
    }
    return NULL;
}
...
void MR_Emit(char *key, char *value) 
{
    ...
    for (int i = 0; i < partitionHashT->numKeys; ++i) {
        printf("this is key: %s\n", partitionHashT->entries[i]->key);
    }
}
...
void MR_Run(int argc, char *argv[], Mapper map, int num_mappers, Reducer reduce, int num_reducers, Partitioner partition) 
{
    ...
    printf("In MrRUN numkeys %lu\n", partitionHashT[partition_number]->numKeys);
    ...
    // create num_reducers threads to perform the reduction tasks
