...
void *worker1(void *arg) {
    int i;
    for (i = 0; i < 1e6; i++) {
	printf("worker 1: start lock acquire\n");
	printf("worker 1: grab lock 1\n");
	Sem_wait(&lock_1); 
	printf("worker 1: grab lock 2\n");
	Sem_wait(&lock_2);
	counter_1++;
	counter_2++;
	Sem_post(&lock_1);
...
void *worker2(void *arg) {
    int i;
    for (i = 0; i < 1e6; i++) {
	printf("worker 2: start lock acquire\n");
	printf("worker 2: grab lock 2\n");
	Sem_wait(&lock_2); 
	printf("worker 2: grab lock 1\n");
	Sem_wait(&lock_1);
	counter_1++;
	counter_2++;
	Sem_post(&lock_2);
